...
const ELEMENTS := ["fire", "frost", "shock", "nature",
	"shadow", "water", "arcane", "thunder"]

## Per-style base feel. Numbers feed CombatFx.impact(shake, hitstop, scale, chroma).
const PROFILES := {
	"slash": {"cue": "impact_slash", "pitch": 1.06, "color": Color(1.0, 0.92, 0.7),
		"shake": 0.09, "hitstop": 0.0, "scale": 1.0, "chroma": 0.18},
	"blunt": {"cue": "impact_thud", "pitch": 0.94, "color": Color(1.0, 0.78, 0.42),
		"shake": 0.13, "hitstop": 0.02, "scale": 0.9, "chroma": 0.24},
	"magic": {"cue": "impact_magic", "pitch": 1.02, "color": Color(0.72, 0.62, 1.0),
		"shake": 0.10, "hitstop": 0.0, "scale": 1.1, "chroma": 0.30},
	"claw": {"cue": "impact_claw", "pitch": 1.0, "color": Color(1.0, 0.4, 0.2),
		"shake": 0.12, "hitstop": 0.02, "scale": 1.05, "chroma": 0.22},
}

## Surface response: how a style reads on each material class.
const SURFACE_MULT := {
	"flesh": {"cue": "", "shake": 1.0, "burst": 1.0, "pitch": 0.96},
	"plant": {"cue": "impact_plant", "shake": 0.9, "burst": 1.15, "pitch": 1.0},
	"stone": {"cue": "impact_stone", "shake": 1.35, "burst": 0.8, "pitch": 1.05},
}
...
## Every element the weapon and enemy data actually use. The four original
## entries left shadow, water, arcane and thunder strikes falling back to a
## generic ember pop, which is why unrelated weapons read as the same hit.
...
static func resolve(style: String, surface: String = "plant") -> Dictionary:
	...
	return {
		"cue": cue,
		"pitch": float(p.get("pitch", 1.0)) * float(sm.get("pitch", 1.0)),
...
static func apply_strike(context: Node, style: String, surface: String,
	...
	if audio != null and not str(profile.cue).is_empty():
		audio.play_cue(str(profile.cue), float(profile.get("pitch", 1.0)))
