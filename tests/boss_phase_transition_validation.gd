...
func _ready() -> void:
	...
	var opening_guidance: Dictionary = boss.phase_guidance()
	_assert_true(not str(opening_guidance.get("pattern", "")).is_empty()
		and not str(opening_guidance.get("safe_zone", "")).is_empty(),
		"boss exposes readable opening pattern and safe-zone guidance")
	_assert_true(str(opening_guidance.get("pattern", "")).contains("Thorn"),
		"Matriarch exposes authored phase-specific opening pattern")
	var phase_label := Label.new()
	boss.boss_phase_label = phase_label
	boss._refresh_boss_bar()
	_assert_true(phase_label.text.contains("Thorn lash and spacing")
		and phase_label.text.contains("red lash arc"),
		"boss bar presents authored opening phase guidance")
	...
	_assert_guidance(boss, 1, "phase two")
...
func _assert_guidance(boss: Node3D, expected_phase: int, label: String) -> void:
	var guidance: Dictionary = boss.phase_guidance()
	_assert_true(int(boss.current_phase) == expected_phase
		and not str(guidance.get("pattern", "")).is_empty()
		and not str(guidance.get("safe_zone", "")).is_empty(),
		"boss exposes pattern and safe-zone guidance after " + label)
